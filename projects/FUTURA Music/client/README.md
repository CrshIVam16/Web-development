...
FUTURA Music is a full‑stack MERN project inspired by modern music platforms. It allows users to **sign up/login**, **discover and play songs**, **upload local MP3 files**, **manage playlists**, and use a **Spotify-like bottom player**. It also supports **external preview tracks** using **Deezer (primary)** and **iTunes (fallback)**.
...
- Recently played history stored per-user in localStorage:
...
- React Router
- Tailwind CSS
- API wrapper using `fetch` (`client/src/utils/api.js`)
...
* songs[] embedded objects (supports local + external)
  * _id is stored as a String so it can support:
